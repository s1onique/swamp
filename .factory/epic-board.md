...
| SWAMP-CHARACTERIZE-REST01 | CLOSED | Verdict: REMAINING_FAILURE_SURFACE_MIXED. Subject `a392c49e`. Natural completion of the full native suite under prewarmed external cache (`/tmp/swamp-char-rest01.ipyvth/deno`) + synthetic writable HOME + native arm64 Deno 2.9.7 + `--parallel`. Runner summary: `FAILED | 12287 passed (214 steps) | 33 failed | 30 ignored (1 step) (10m11s)`. Conservation holds (passed + failed + ignored = total). 33 failures → 3 clusters: CLUSTER-01 (31 ENVIRONMENTAL/FALSIFIED_BY_CONTROL, "deno not on PATH" — passes 51/0 with deno on PATH), CLUSTER-02 (1 ENVIRONMENTAL/REPRODUCED_REPEATEDLY, parallel-load RPC channel close — passes 5/0 isolated), CLUSTER-03 (1 PROJECT_DEFECT/REPRODUCED_REPEATEDLY, doctor portable terminal-race timing edge — same single failure on N=3 isolated runs). unclassified=0, unresolved=0, no_unknown_red=true, NO_PRODUCTION_CODE_CHANGED. New helper: `.factory/scripts/parse_deno_test_result.ts` + 12 self-tests. New verifier: `.factory/scripts/check_characterize_rest01.sh` — 23/23 PASS. Recommend next ACT: `SWAMP-DOCTOR-PORTABLE-TIMING01`. |
